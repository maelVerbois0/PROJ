...
heuristic_relaxed_repair_static(data)
filenames = get_file()
fichier_resultats = "results/logs/resultats_experimentation_heuristique_statique.csv"
...
open(fichier_resultats, "w") do io
    ...
    for file in filenames
        ...
        res = heuristic_relaxed_repair_static(data)
        ...
        if res.termination_status != "NOTFOUND"
            plot_result(data, res, String(split(file, ".")[1]), save_path = "results/figs/heuristique_statique/$(String(split(file, ".")[1]))")
        end
    end
end
